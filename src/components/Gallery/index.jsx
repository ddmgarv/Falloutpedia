  ...
  render() {
    ...
    return (
      <section className="galleryWrapper">
        <h2>
          Haga click en la caratula de cada juego para desplegar la galería
        </h2>
        <div className="frontCovers">
          {fwp.map((fallout, index) => (
            <div
              className="frontCovers__item"
              onClick={() => this.renderGallery(fallout)}
              key={index}
            >
              <img src={fallout} alt="" />
            </div>
        ...
        {galDisplayStatus && (
          <Gallery content={galContent} exitGal={() => this.unsetGallery()} />
        )}
      </section>
    );
  }
